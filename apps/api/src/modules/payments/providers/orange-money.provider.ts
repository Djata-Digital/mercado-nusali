...
export class OrangeMoneyProvider implements IPaymentProvider {
  readonly providerName = PaymentProvider.ORANGE;
  readonly integrationMode = PaymentProviderIntegrationMode.SIMULATED;
  readonly supportedMethods = [PaymentMethodType.ORANGE_MONEY] as const;
  ...
  async createPayment(
    ...
    const txId = `OM-${crypto.randomBytes(6).toString('hex').toUpperCase()}`;
    const ref = `OM-REF-${paymentId.slice(0, 8)}`;
    ...
    return {
      ...
      metadata: {
        paymentUrl: `https://api.orange.com/om/pay/${txId}`,
        ussdPrompt: `*144*4*${amount}#`,
  ...
  async refundPayment(
    ...
    return {
      ...
      providerTransactionId: `OM-REFUND-${crypto.randomBytes(4).toString('hex')}`,
    };
  }
